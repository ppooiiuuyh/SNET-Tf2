...
from models.model_train_augcyclegan import Model_Train
from data_utils import *
from ops import *
from pretrained_line_normalizer.line_normalizer_model_inference import  Model_Tester as Line_Normalizer
...
def generate_expname_automatically():
    name = "line_stylizer_iccv_%s_%02d_%02d_%02d_%02d_%02d" % (config.model_tag,
...
---------------------------------------------------------------------"""
""" build model """
line_normalizer = Line_Normalizer(config.channels)
...
train_iterator, test_dataset, reference_iterator = make_iterator_ontime(config, line_normalizer= line_normalizer) #load each batches ontime
...
while True : #manuallry stopping
    ...
    if model.step.numpy() % 100 == 0:
        log = model.test_step(test_dataset,reference_iterator, summary_name="test")
